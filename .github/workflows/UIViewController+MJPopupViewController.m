...
- (void)setNewPopupPosition:(CGRect)position {

    [self setNewPopupPosition:position animated:false];
}

- (void)setNewPopupPosition:(CGRect)position animated:(BOOL)animated {
    
    UIView *popupView = [self topView];
//    UIView *popupView = [sourceView viewWithTag:kMJPopupViewTag];
//    UIView *overlayView = [sourceView viewWithTag:kMJOverlayViewTag];
...
//    [popupView setFrame:CGRectMake(10, 10, size.width, size.height)];
    ...
    if (animated) {
        [UIView animateWithDuration:0.2f
                         animations:^ {
                             [popupView setFrame:CGRectMake(position.origin.x, position.origin.y, position.size.width, position.size.height)];
                         }
                         completion:^(BOOL finished) {
                             // to nothing
                         }
...
- (void)setNewPopupSize:(CGSize)size animated:(BOOL)animated {
    ...
    if (animated) {
        [UIView animateWithDuration:0.2f
                         animations:^ {
                             [overlayView setFrame:CGRectMake(point.x, point.y, size.width, size.height)];
                         }
                         completion:^(BOOL finished) {
                             // to nothing
                         }
